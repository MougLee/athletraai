...
val mainClassName = "ai.athletra.athletraai.Main"
...
lazy val commonSettings = Seq(
  organization := "ai.athletra.athletraai",
  scalaVersion := "3.7.2",
  // version
...
lazy val backend: Project = (project in file("backend"))
  ...
  .settings(
    ...
    generateOpenAPIDescription := Def.taskDyn {
      ...
      Def.task {
        (Compile / runMain).toTask(s" ai.athletra.athletraai.writeOpenAPIDescription $targetPath").value
      }
    }.value,
  ...
  .settings(
    ...
    buildInfoPackage := "ai.athletra.athletraai.version",
